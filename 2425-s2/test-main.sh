...
if [ ! -e $MAIN.class ]
then
	echo "$MAIN.class does not exist.  Have you compiled it with make or javac?"
	exit 1
fi
...
do
	...
	then
		...
		then
			out=$(mktemp -t $PROG)
			gtimeout 5s java "$MAIN" < "inputs/$PROG.$i.in" | ghead -c 50MB > "$out"
		else
			out=$(mktemp --suffix=$PROG)
			timeout 5s java $MAIN < inputs/$PROG.$i.in | head -c 50MB > $out
		fi
		status="$?"
		if [ "$status" -ne "0" ]
		then
